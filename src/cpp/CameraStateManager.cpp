...
// CameraStateManager
//
// ERCameraState is the single TESCameraState subclass ER installs into
// PlayerCamera.  It dispatches Begin/End/Update to whichever logical camera
// state is currently active (s_activeHandle).
//
// Lifecycle
// ---------
//   Init()                      — allocates ERCameraState once at plugin load
//   ActivateLogicalCameraState  — installs ERCameraState into PlayerCamera
...
//   Restoring vanilla TPS drops PlayerCamera's reference; ER's copy keeps
//   the object alive for future re-activation.
//
// Engine eviction invariant (Issue 1)
// ------------------------------------
//   s_activeHandle is valid if and only if ERCameraState is the current
//   PlayerCamera state.  ERCameraState::End() clears the active handle and
...
namespace EngineRelay::CameraStateManager {
    ...
    //
    // These are std::atomic so ERCameraState::End() (render thread) can clear
    // them safely.  The preemption stack below is NOT atomic — only game-thread
    // code may access it (under s_camMutex).

    static std::atomic<ERCameraHandle>  s_activeHandle{ kInvalidERCameraState };
    ...
    using PreemptionEntry = std::pair<ERCameraHandle, std::uint32_t>;
    ...
    // =========================================================================
    // ERCameraState — the singleton TESCameraState subclass
    //
    ...
    class ERCameraState : public RE::TESCameraState {
    public:
        ...
        void Begin() override
        {
            const ERCameraHandle h =
                s_activeHandle.load(std::memory_order_relaxed);
            if (h == kInvalidERCameraState) return;
        ...
        // ER game-thread API (e.g. ActivateLogicalCameraState) from within endFn.
        void End() override
        {
            const ERCameraHandle h =
                s_activeHandle.load(std::memory_order_relaxed);
            ...
            // and only if ERCameraState is the current PlayerCamera state.
            ...
            s_activeHandle.store(kInvalidERCameraState, std::memory_order_relaxed);
        ...
        void Update(RE::BSTSmartPointer<RE::TESCameraState>& /*a_next*/) override
        {
            ...
            if (def.callbacks.updateFn) {
                ...
                // ERCameraState is active we must do it ourselves.
    ...
    /// Restore the vanilla ThirdPersonState in PlayerCamera if ERCameraState
    ...
    /// we installed ERCameraState.  Clears s_savedTPS after use.
    static void RestoreVanillaTPS()
    {
        auto* pcam = RE::PlayerCamera::GetSingleton();
        if (!pcam || !s_bsbCameraState) return;

        // Only restore if ERCameraState is actually the current state.
    ...
    void Init()
    {
        ...
        LOG_INFO("CameraStateManager: ERCameraState allocated (id=kAnimated).");
    }
        ...
        std::uint32_t                 priority)
    {
        ...
        for (std::size_t i = 0; i < s_cameraStates.size(); ++i) {
            if (s_cameraStates[i].modName == modName) {
                ...
                return static_cast<ERCameraHandle>(i);
            }
        }

        const ERCameraHandle handle =
            static_cast<ERCameraHandle>(s_cameraStates.size());
    ...
    {
        const ERCameraHandle currentHandle =
            s_activeHandle.load(std::memory_order_relaxed);
        if (currentHandle == kInvalidERCameraState) return true;
    ...
    {
        if (handle == kInvalidERCameraState) {
        ...
        if (currentHandle != kInvalidERCameraState &&
        ...
        // Install ERCameraState into PlayerCamera if not already current.
        auto* pcam = RE::PlayerCamera::GetSingleton();
        if (pcam && s_bsbCameraState) {
            ...
            if (currentHandle == kInvalidERCameraState && !s_savedTPS) {
                s_savedTPS = pcam->cameraStates[RE::CameraState::kThirdPerson];
            }
            // Set the camera back-pointer on ERCameraState so Begin()/Update()/End()
            ...
            // externally-allocated states like ERCameraState.  The old per-mod
    ...
    {
        // If the engine already evicted ERCameraState (End() cleared the active
        ...
        if (s_activeHandle.load(std::memory_order_relaxed) == kInvalidERCameraState) {
            std::lock_guard lk(s_camMutex);
            if (!s_preemptionStack.empty()) {
        ...
        PreemptionEntry top{ kInvalidERCameraState, 0 };
        {
            std::lock_guard lk(s_camMutex);
            if (!s_preemptionStack.empty()) {
        ...
        if (top.first != kInvalidERCameraState) {
            // Restore the preempted lower-priority state.
            // ERCameraState stays in PlayerCamera — no TPS swap needed.
        ...
        // ERCameraState::End() will fire inside RestoreVanillaTPS() → SetState(),
        // but by then s_activeHandle is already kInvalidERCameraState so
        // it becomes a no-op — this explicit dispatch is the authoritative one.
        {
            const ERCameraHandle dying =
                s_activeHandle.load(std::memory_order_relaxed);
            if (dying != kInvalidERCameraState) {
                // s_cameraStates is append-only after kPostLoad — safe without lock.
