...
namespace EngineRelay::CharacterStateAllocator {
    ...
    // embedded ERPhysicsCallbacks with a lean ActorHostData pointer.
    // =========================================================================
    ...
    struct ActorHostData {
        std::atomic<ERStateHandle> activeHandle{ kInvalidERState };
        std::atomic<ERStateHandle> preemptedHandle{ kInvalidERState };
    ...
    static const LogicalStateDef* GetActiveDef(const ActorHostData* hostData)
    {
        ...
        const ERStateHandle h = hostData->activeHandle.load(std::memory_order_relaxed);
        if (h == kInvalidERState) return nullptr;
        ...
        for (std::size_t i = 0; i < s_logicalStates.size(); ++i) {
            if (s_logicalStates[i].modName == modName) {
                LOG_WARN("CharacterStateAllocator: logical state '{}' already registered "
                         "at handle {}.", modName, i);
                return static_cast<ERStateHandle>(i);
            }
        }

        const ERStateHandle handle =
            static_cast<ERStateHandle>(s_logicalStates.size());
                                 ...
                                 std::uint32_t priority)
    {
        ...
        const ERStateHandle currentHandle =
            hostData->activeHandle.load(std::memory_order_relaxed);
        if (currentHandle == kInvalidERState) return true;
    ...
    {
        if (!controller) return false;
        if (handle == kInvalidERState) return false;
        ...
        if (currentHandle != kInvalidERState &&
            ...
            incomingPriority > currentPriority)
        {
            ...
            hostData->preemptedHandle.store(kInvalidERState, std::memory_order_relaxed);
        }
    ...
    void DeactivateLogicalState(RE::bhkCharacterController* controller)
    {
        ...
        if (it != s_actorEntries.end() && it->second.hostData) {
            ...
            const ERStateHandle preempted =
                hostData->preemptedHandle.load(std::memory_order_relaxed);

            if (preempted != kInvalidERState) {
            ...
            hostData->activeHandle.store(kInvalidERState, std::memory_order_relaxed);
    ...
    {
        if (!controller) return kInvalidERState;
        std::lock_guard lock(s_actorMutex);
        auto it = s_actorEntries.find(controller);
        if (it == s_actorEntries.end() || !it->second.hostData)
            return kInvalidERState;
