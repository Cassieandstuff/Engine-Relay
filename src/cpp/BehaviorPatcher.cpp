...
namespace EngineRelay::BehaviorPatcher {
    ...
    // -----------------------------------------------------------------------
    // Pending ER event — bridges SendEvent → ER_Activate (option-B path)
    //
    // ER_API::SendEvent stores the triggering event name here immediately
    // before calling actor->NotifyAnimationGraph().  Since ER_Activate fires
    // synchronously within that call (proven by log ordering), ER_Activate
    ...
    static int32_t AppendEvent(RE::hkbBehaviorGraphData* graphData, const char* eventName)
    {
        ...
        auto* info = GrowHkArray<EREventInfo>(&graphData->eventInfos);
        if (!info) {
    ...
    static int32_t GetNextStateId(RE::hkbStateMachine* sm)
    {
        ...
        for (int32_t i = 0; i < sm->states.size(); ++i) {
            ...
            auto* realState = reinterpret_cast<ERStateInfo*>(stateInfo);
    ...
    static ERBehaviorReferenceGenerator* BuildBehaviorRef(
    ...
    static void AppendStateToSM(RE::hkbStateMachine* rootSM, ERStateInfo* stateInfo)
    {
        ...
        int32_t eventIdx, int32_t stateId)
    {
        ...
        auto* existingTIA = reinterpret_cast<ERTransitionInfoArray*>(
        ...
        if (existingTIA->transitions_data && oldSize > 0) {
            std::memcpy(newData, existingTIA->transitions_data,
                oldSize * sizeof(ERTransitionInfo));
        }
        ...
        std::string_view graphName, uint32_t startIndex)
    {
        ...
        for (uint32_t i = startIndex; i < regCount; ++i) {
            ...
            const std::string nodeNameStr = "ER_" + reg.modName;
            ...
            // ── 3. Build StateInfo ──
            const std::string stateNameStr = "ER_State_" + reg.modName;
        ...
        //      Extend numStaticNodes by kERNodeReservation slots per injected
        ...
        //   2. In ER_Activate (when the sub-behavior graph is first activated):
        ...
        //
        // kERNodeReservation = 512 per registration: generous ceiling for any single
        // sub-behavior file.  Cost: sizeof(nodePartitionInfo entry) × 512 bytes extra.
        static constexpr std::uint16_t kERNodeReservation = 512;
        const std::uint32_t totalStatic =
            static_cast<std::uint32_t>(nextNodeId) +
            static_cast<std::uint32_t>(injectedCount) * kERNodeReservation;
        ...
        // ER_Activate walk starts assigning from the base of the reserved range.
        ...
        LOG_INFO("BehaviorPatcher: reserved {} static partition slots ({} sub-behavior(s), "
            ...
            static_cast<std::uint32_t>(injectedCount) * kERNodeReservation,
            injectedCount, kERNodeReservation,
            graph->numStaticNodes, graph->nextUniqueID);
    ...
    //
    // ER reserves a static ID range in the parent graph via numStaticNodes
        ...
        std::unordered_set<void*>&   visited)
    {
        ...
        if (vtable == vtSM) {
            ...
            for (auto* statePtr : sm->states) {
                if (statePtr) {
                    auto* si = reinterpret_cast<ERStateInfo*>(statePtr);
            ...
            auto* cb = reinterpret_cast<ERCyclicBlendGen*>(gen);
    ...
    /// Returns true if graphName identifies a ER-managed switchboard graph.
    ...
    {
        return graphName == "EngineRelay.hkb" ||
               graphName == "DragonEngineRelay.hkb";
    }

    using ActivateFn = void(*)(RE::hkbBehaviorGraph*, const RE::hkbContext&);
    static ActivateFn s_originalActivate = nullptr;

    static void ER_Activate(RE::hkbBehaviorGraph* a_this, const RE::hkbContext& a_context)
    {
        ...
        // "EngineRelay.hkb" instead of "0_Master.hkb").
        //
        ...
        if (!isRootGraph && !hasMatchingReg) {
            ...
            // Switchboard graphs (EngineRelay.hkb, DragonEngineRelay.hkb,
            ...
            const bool isERGraph = graphName && IsSwitchboardGraph(graphName);
            uint32_t regCount = 0;
            ...
            // Option B: EngineRelay.hkb uses a pre-patched HKX (served by
            ...
            if (isERGraph) {
                ...
                //   stateId=0  → ER_IdleState   (base/idle placeholder)
                //   stateId=1  → first ER-targeting registration
                //   stateId=2  → second ER-targeting registration
                //   ...
                // SetPendingEREvent was called just before NotifyAnimationGraph so
    ...
    void Install()
    {
        ...
        // Fires before Havok builds stateIDToIndexMap. ER injects the structural
        ...
        s_originalActivate = reinterpret_cast<ActivateFn>(vtbl.write_vfunc(0x04, &ER_Activate));
