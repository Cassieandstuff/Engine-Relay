...
// =============================================================================
// ERMemoryStream — read-only in-memory BSResource::Stream
//
...
class ERMemoryStream : public RE::BSResource::Stream
{
public:
    ...
    explicit ERMemoryStream(std::vector<std::uint8_t> a_data)
    ...
    ~ERMemoryStream() override = default;
    ...
    {
        a_out = RE::BSTSmartPointer<RE::BSResource::Stream>{ new ERMemoryStream(m_data) };
    }
...
private:
    ...
    explicit ERMemoryStream(std::shared_ptr<const std::vector<std::uint8_t>> a_data)
        : RE::BSResource::Stream(static_cast<std::uint32_t>(a_data->size()))
